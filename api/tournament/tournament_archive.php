...
try {
  ...
  if ((int)($row['is_archived'] ?? 0) === 1) {
    $pdo->commit();
    respond_json(['ok' => true, 'id' => $id, 'already_archived' => true]);
  }

  $st = $pdo->prepare(
    'UPDATE tournaments SET is_archived = 1, archived_at = NOW(), is_current = 0 WHERE id = :id'
  );
  $st->execute([':id' => $id]);
  ...
  respond_json(['ok' => false, 'error' => 'Archive failed', 'details' => $e->getMessage()], 500);
}
